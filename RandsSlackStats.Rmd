...
start4weeks <- latestDate - dweeks(4.5)
earliestDate <- min(df$start)
```

# Stats for last week i
Week starting `r max(df$start)`.

## Top 20 channels and users
```{r}
plotDataLastWeek <- df %>% filter(start > latestDate - dweeks(1.1))  

df.channelsLastWeek <- df %>% filter(start > latestDate - dweeks(1.1)) %>% group_by(channel) %>% summarise(numPosts = sum(numPosts)) %>% 
...
df.usersLastWeek <- df %>% filter(start > latestDate - dweeks(1.1)) %>% group_by(user) %>% summarise(numPosts = sum(numPosts)) %>% 
  arrange(numPosts) %>% top_n(20)
df.usersLastWeek %>% 
